...
2 — формально ответил, без вовлечения, упустил очевидную возможность
3 — норма для продажного звонка: ответил на вопросы, но не развил диалог
4 — хорошо: точно ответил на конкретный вопрос / профессионально провёл B2B-звонок / правильно отработал нестандартную ситуацию
5 — отлично: выявил потребности, предложил варианты, обработал возражения, договорился о следующем шаге

ВАЖНЫЕ ПРАВИЛА:
- Если это ошибочный звонок (wrong_number) и менеджер вежливо объяснил — оценка 4.
- Если это B2B/партнёрский звонок (агентство, партнёр, контрагент) и менеджер дал чёткие профессиональные ответы — оценка 4.
- Если клиент задал конкретный вопрос и получил точный ответ за короткое время — оценка 4.
- Если исходящий звонок и клиент сказал что занят/недоступен — исход client_unavailable, оценка 3, никаких issues.
- НЕ штрафуй за "не уточнил даты/количество" если у клиента не было запроса на тур.
